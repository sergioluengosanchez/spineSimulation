...
This computation can take several days so we provide the resulting model. Also enable the `scale` flag can increase the computational time considerably. We provide the resulting probabilistic model as a binary R object so interested user can reproduce the experiments just running the below examples: 
